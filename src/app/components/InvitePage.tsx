...
import { Star, MapPin, Calendar, Clock, Plus, Trash2, Sparkles, AlertCircle } from 'lucide-react';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (confirmation === 'sim') {
      const invalidParticipant = participants.find((p) => {
        ...
        if (p.isChild && (p.age === null || p.age < 0)) return true;
        return false;
      });
    ...
    saveRSVP({
      ...
      participants:
        confirmation === 'sim'
          ? participants.map((p) => ({
              name: p.name.trim(),
              age: p.isChild && p.age !== null ? p.age : null,
              isChild: p.isChild,
            }))
          : [],
    })
                  ...
                  <div>
                    <p className="text-sm text-amber-800">
                      <span className="font-bold italic">
                        Nota: Necessário preenchimento das informações e confirmação de presença para acesso ao local do evento.
                      </span>
                    </p>
                  </div>
                </div>
                        ...
                        >
                          ...
                          <div className="w-32">
                            <select
                              ...
                              onChange={(e) =>
                                updateParticipant(index, 'isChild', e.target.value)
                              }
                          ...
                          {/* Idade - SOMENTE visível para crianças */}
                          {participant.isChild && (
                            <div className="w-24">
                              <select
